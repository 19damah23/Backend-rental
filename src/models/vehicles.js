...
const getVehiclesByCategory = (category) => new Promise((resolve, reject) => {
  conn.query(`SELECT * FROM vehicles WHERE category = '${category}'`, (error, result) => {
    if (!error) {
      resolve(result);
    } else {
      reject(error);
...
const getVehiclesByCategoryWithPaginate = (category, orderBy, sortBy, perPage, offset) => new Promise((resolve, reject) => {
  conn.query(`SELECT * FROM vehicles WHERE category = '${category}' ORDER BY ${orderBy} ${sortBy} LIMIT ${perPage} OFFSET ${offset}`, (error, result) => {
    if (!error) {
      resolve(result);
    } else {
      reject(error);
...
module.exports = {
  ...
  getVehiclesByCategoryWithPaginate,
};
